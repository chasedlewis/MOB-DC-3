...
//        let swipe = UISwipeGestureRecognizer(target: self, action: "gestureAction") // Creates a new swipe gesture
...
//        label.userInteractionEnabled = true
        ...
        view.addGestureRecognizer(tap) // Allows the user to tap anywhere on the screen as opposed to just on the label like the code above does
    }
    
    func gestureAction() {
        println("Gesture Performed")
        self.performSegueWithIdentifier("showModal", sender: nil) // Connects the segue to the gesture
    }
